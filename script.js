...
async function toggleScreenShare() {
    ...
    try {
        ...
        // Replace camera track on existing senders AND add as new track for overlay
        state.peers.forEach(pc => {
            const vs = pc.getSenders().find(s => s.track?.kind === "video");
            if (vs) {
                vs.replaceTrack(screenTrack);
            }
            // Add as second track so receiver gets a new ontrack for the overlay
